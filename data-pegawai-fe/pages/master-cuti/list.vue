    ...
    <b-card title="Master GPS">
      ...
      <b-table-simple responsive>
        <b-thead head-variant="dark">
          <b-tr>
            <b-th>No</b-th>
            <b-th>Nama Jenis Cuti</b-th>
            <b-th>Durasi (hari)</b-th>
            <b-th>Status</b-th>
          </b-tr>
        </b-thead>
        <b-tbody>
          <template v-for="(item, i) in datalist">
            <b-tr :key="i">
              <b-td>{{i+1}}</b-td>
              <b-td>{{ item.name }}</b-td>
              <b-td>{{ item.duration_day }}</b-td>
              <b-td>
                <b-form-checkbox v-model="item.is_active" name="check-button" switch @change="updateItem(item)" v-if="user.role_id == 1"></b-form-checkbox>
                <span v-if="user.role_id != 1 && item.is_active">Aktif</span>
                <span v-if="user.role_id != 1 && !item.is_active">Tidak Aktif</span>
              </b-td>
            </b-tr>
          </template>
        </b-tbody>
...
export default {
  ...
  methods: {
    async loadData(){
      ...
      let datalist = [];
      let datacount = 0;
      let f1 = await axios.get("/master-cuti", {
        params: {
    ...
    pageLink(page) {
      ...
      return { path: "/master-cuti/" + stringifyParams(searchparams) };
    },
    sortLink(sort) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    promptSearch() {
      ...
      this.$router.replace({
        path: "/master-cuti/" + stringifyParams(searchparams),
      });
    },
    setFilter(col, val) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    async deleteItem(data, index){
      ...
      await axios.post("/master-cuti/delete", {
            id: data.id,
          })
          .then(({ data }) => {
