<template>
  <b-card title="Edit Bidang" class="my-4">
    <b-container fluid>
      <b-row class="my-1">
        <b-col sm="3">
...
export default {
  ...
  head() {
    return { title: "Edit Bidang" }
  },
  async asyncData({ params }) {
    let f2 = axios.get('bidang/' + params.id)
  ...
  methods: {
    async save() {
      this.loading = true
      try {
        let resp = (await axios.patch('bidang/' + this.$route.params.id, this.form)).data
        if (resp.success) {
          Swal.fire({
            icon: 'success',
            title: 'Berhasil',
