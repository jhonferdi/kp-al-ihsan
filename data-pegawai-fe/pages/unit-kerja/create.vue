<template>
  <b-card title="Unit Kerja Baru" class="my-4">
    <br>
    <b-container fluid>
      <b-row class="my-2">
        <b-col sm="3">
...
export default {
  ...
  head() {
    return { title: "Create Unit Kerja" }
  },
  ...
  methods: {
    async save() {
      this.loading = true
      try {
        let resp = (await axios.post('unit-kerja', this.form)).data
        if (resp.success) {
          Swal.fire({
            icon: 'success',
            title: 'Berhasil',
          ...
          this.form.unit_kerja_nama = ''
          this.form.unit_kerja_level = ''
          this.form.unit_kerja_parent = ''
