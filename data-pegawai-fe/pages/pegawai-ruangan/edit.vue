<template>
  <b-card title="Edit Pegawai Ruangan" class="my-4">
    <b-container fluid>
      <b-row class="my-1">
        <b-col sm="3">
        ...
        <b-col sm="9">
          <v-select id="input-ruangan" :options="RIDOptions" :reduce="item => item.ruangan_id" label="nama_ruangan"
            placeholder="Pilih Ruangan" v-model="form.ruangan_id"
            :class="{ 'is-invalid': !getErrorState('ruangan_id') }">
          </v-select>
        ...
        <b-col sm="9">
          <v-select id="input-role" :options="['Kepala Ruangan', 'Admin Ruangan', 'Pegawai']" label="role"
            placeholder="Pilih Role" v-model="form.role" :class="{ 'is-invalid': !getErrorState('role') }">
          </v-select>
...
export default {
  ...
  head() {
    return { title: "Edit Pegawai Ruangan" }
  },
  async asyncData({ params }) {
    let f2 = axios.get('pegawai-ruangan/' + params.id)
  ...
  methods: {
    async save() {
      this.loading = true
      try {
        let resp = (await axios.patch('pegawai-ruangan/' + this.$route.params.id, this.form)).data
        if (resp.success) {
          Swal.fire({
            icon: 'success',
            title: 'Berhasil',
