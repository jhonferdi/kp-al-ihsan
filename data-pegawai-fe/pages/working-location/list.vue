      ...
      <b-table-simple responsive>
        <b-thead head-variant="dark">
          <b-tr>
            ...
            <b-th>Latitude</b-th>
            <b-th>Longitude</b-th>
            <b-th style="width: 100px">Aksi</b-th>
          </b-tr>
        </b-thead>
        <b-tbody>
          <template v-for="(item, i) in datalist">
            <b-tr :key="i">
              ...
              <b-td>{{ item.lng }}</b-td>
              <b-td>
                <nuxt-link :to="'/working-location/update/'+item.id">
...
export default {
  ...
  methods: {
    async loadData(){
      ...
      let datalist = [];
      let datacount = 0;
      let f1 = await axios.get("/working-locations", {
        params: {
    ...
    pageLink(page) {
      ...
      return { path: "/working-location/" + stringifyParams(searchparams) };
    },
    sortLink(sort) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    promptSearch() {
      ...
      this.$router.replace({
        path: "/working-location/" + stringifyParams(searchparams),
      });
    },
    setFilter(col, val) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    async deleteItem(data, index){
      ...
      await axios.post("/working-locations/delete", {
            id: data.id,
          })
          .then(({ data }) => {
