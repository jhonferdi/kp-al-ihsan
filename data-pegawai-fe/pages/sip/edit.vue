...
export default {
  ...
  async asyncData({ params }) {
    ...
    const f3 = await axios.get('sip/' + params.id)
    const f3resp = (await f3).data
    const form = f3resp.model
    return {
      PEGOptions,
      form
    }
  ...
  methods: {
    async save() {
      this.loading = true
      try {
        let resp = (await axios.patch('sip/' + this.$route.params.id, this.form)).data
        if (resp.success) {
          Swal.fire({
            icon: 'success',
            title: 'Berhasil',
