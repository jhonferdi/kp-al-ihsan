    ...
    <b-card title="Pengumuman">
      <div class="form-group row">
        <div class="col-sm-12 text-right">
          <nuxt-link :to="'/pengumuman/create'">
            <button class="btn btn-success btn-sm"><i class="fas fa-plus" style="color: #fff" /> Tambah Pengumuman</button>
          </nuxt-link>
        </div>
      </div>
      <div class="form-group row">
        ...
        <b-tbody>
          <template v-for="(item, i) in datalist">
            <b-tr :key="i">
              ...
              <b-td>{{ item.date_time_formatted }}</b-td>
              <b-td>{{ item.status }}</b-td>
              <b-td>
                <nuxt-link :to="'/pengumuman/update/'+item.id">
                  <b-button variant="warning" size="sm">
                    <b-icon icon="pencil"></b-icon>
                  </b-button>
                </nuxt-link>
                <b-button variant="info" size="sm" @click="sendItem(item, i)" v-if="item.status == 'draft'">
...
export default {
  ...
  methods: {
    async loadData(){
      ...
      let datalist = [];
      let datacount = 0;
      let f1 = await axios.get("/announcements", {
        params: {
    ...
    pageLink(page) {
      ...
      return { path: "/pengumuman/" + stringifyParams(searchparams) };
    },
    sortLink(sort) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    promptSearch() {
      ...
      this.$router.replace({
        path: "/pengumuman/" + stringifyParams(searchparams),
      });
    },
    setFilter(col, val) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    async sendItem(data, index){
      console.log(data)
      let confirmation
      await Swal.fire({
        title: 'Apakah anda yakin?',
        text: "Mengirim " + data.title,
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#146EBE',
      ...
      await axios.post("/announcements/send", {
            id: data.id,
          })
          .then(({ data }) => {
