    ...
    <b-card title="Konfigurasi Jam Kerja Pegawai">
      <div class="form-group row">
        <div class="col-sm-12 text-right">
          <nuxt-link :to="'/konfigurasi-jam-kerja-pegawai/create'">
      ...
      <b-table-simple responsive>
        ...
        <b-tbody>
          <template v-for="(item, i) in datalist">
            <b-tr :key="i">
              <b-td>{{i+1}}</b-td>
              <b-td>{{ item.pegawai.peg_nip }}</b-td>
              <b-td>{{ item.pegawai.peg_nama }}</b-td>
              <b-td>{{ item.pegawai.satuan_kerja ? item.pegawai.satuan_kerja.satuan_kerja_nama : '-' }}</b-td>
              <b-td>{{ item.pegawai.unit_kerja ? item.pegawai.unit_kerja.unit_kerja_nama : '-' }}</b-td>
              <b-td>{{ item.working_hour.name }}</b-td>
              <b-td>{{ item.status ? 'Aktif' : 'Tidak Aktif' }}</b-td>
              <b-td>
                <nuxt-link :to="'/konfigurasi-jam-kerja-pegawai/update/'+item.id">
...
export default {
  ...
  methods: {
    async loadData(){
      ...
      let datalist = [];
      let datacount = 0;
      let f1 = await axios.get("/konfigurasi-jam-kerja-pegawai", {
        params: {
    ...
    pageLink(page) {
      ...
      return { path: "/konfigurasi-jam-kerja-pegawai/" + stringifyParams(searchparams) };
    },
    sortLink(sort) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    promptSearch() {
      ...
      this.$router.replace({
        path: "/konfigurasi-jam-kerja-pegawai/" + stringifyParams(searchparams),
      });
    },
    setFilter(col, val) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    async deleteItem(data, index){
      ...
      await Swal.fire({
        ...
        text: "Menghapus " + data.pegawai.peg_nama + ' - ' + data.pegawai.peg_nip,
        icon: 'warning',
      ...
      await axios.post("/konfigurasi-jam-kerja-pegawai/delete", {
            id: data.id,
          })
          .then(({ data }) => {
