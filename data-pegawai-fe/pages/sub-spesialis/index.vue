    ...
    <div class="card">
      <div class="card-body">
        <div class="d-flex justify-content-end mb-4">
          <div>
            <b-button variant="outline-success" :to="{ name: 'sub-spesialis.create' }">
              <b-icon icon="plus"></b-icon> Tambah Sub Spesialis
            </b-button>
          </div>
        </div>
        <b-row class="my-1">
          <b-col sm="6">
            <div class="d-flex align-items-center mb-3">
              <div class="mr-4 text-nowrap">Cari Sub Spesialis</div>
              <b-form-input class="w-100" v-model="ctx.filter" placeholder="Masukan Nama Sub Spesialis">
              </b-form-input>
            </div>
          </b-col>
        </b-row>
        <b-table hover responsive id="sub-spesialis-table" ref="table" :items="itemsProvider"
          ...
          <template #cell(actions)="row">
            <nuxt-link :to="{ name: 'sub-spesialis.edit', params: { id: row.item.sub_spesialis_id }}" class="mr-2">
        ...
        <b-row>
          <b-col sm="6">
            <b-pagination v-model="ctx.currentPage" :total-rows="sub_spesialis_count" :per-page="ctx.perPage"
              aria-controls="sub-spesialis-table">
            </b-pagination>
          </b-col>
          <b-col sm="6" class="d-flex justify-content-end">
            <span class="font-size">Total Data : {{ sub_spesialis_count }}</span>
          </b-col>
        </b-row>
      </div>
    </div>
...
export default {
  ...
  head() {
    return { title: 'Sub Spesialis' }
  },

  async asyncData() {
    if (window.tablectxsubspes) {
      var ctx = window.tablectxsubspes
    } else {
      var ctx = {
        currentPage: 1,
        perPage: 20,
    ...
    let sub_spesialis = f1resp.data
    let sub_spesialis_count = f1resp.count
  ...
  data: () => ({
    fields: [
      { key: 'sub_spesialis_nama', label: 'Nama Sub Spesialis', sortable: true, sortDirection: 'asc' },
