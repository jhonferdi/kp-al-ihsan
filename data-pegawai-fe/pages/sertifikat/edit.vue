...
export default {
  ...
  methods: {
    async loadData() {
      ...
      const f2 = await axios.get('sertifikat/' + this.$route.params.id)
      ...
      this.form.peg_id = data.peg_id
      this.form.kategori = data.kategori
      this.form.judul_sertifikat = data.judul_sertifikat
      this.form.tanggal_aktif = data.tanggal_aktif
      this.form.sertifikatOld = data.image_sertifikat
    ...
    async save() {
      ...
      await axios.post('sertifikat/update/' + this.$route.params.id,
        formData,
        {
          headers: {
