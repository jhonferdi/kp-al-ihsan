    ...
    <b-card title="Konfigurasi Jam Kerja Dengan Satuan Kerja">
      <div class="form-group row">
        <div class="col-sm-12 text-right">
          <nuxt-link :to="'/konfigurasi-jam-kerja-opd/create'">
        ...
        <b-tbody>
          <template v-for="(item, i) in datalist">
            <b-tr :key="i">
              ...
              <b-td>
                <nuxt-link :to="'/konfigurasi-jam-kerja-opd/update/'+item.id">
...
export default {
  ...
  methods: {
    async loadData(){
      ...
      let datalist = [];
      let datacount = 0;
      let f1 = await axios.get("/konfigurasi-jam-kerja-opd", {
        params: {
    ...
    pageLink(page) {
      ...
      return { path: "/konfigurasi-jam-kerja-opd/" + stringifyParams(searchparams) };
    },
    sortLink(sort) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    promptSearch() {
      ...
      this.$router.replace({
        path: "/konfigurasi-jam-kerja-opd/" + stringifyParams(searchparams),
      });
    },
    setFilter(col, val) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    async deleteItem(data, index){
      ...
      await Swal.fire({
        ...
        text: "Menghapus " + data.satuan_kerja.satuan_kerja_nama,
        icon: 'warning',
      ...
      await axios.post("/konfigurasi-jam-kerja-opd/delete", {
            id: data.id,
          })
          .then(({ data }) => {
