    ...
    <div class="card">
      <div class="card-body">
        <div class="d-flex justify-content-end mb-4">
          <div>
            <b-button variant="outline-success" :to="{ name: 'tenaga-kerja.create' }">
              <b-icon icon="plus"></b-icon> Tambah Tenaga Kerja
            </b-button>
          </div>
        </div>
        <b-row class="my-1">
          <b-col sm="6">
            <div class="d-flex align-items-center mb-3">
              <div class="mr-4 text-nowrap">Cari Tenaga Kerja</div>
              <b-form-input class="w-100" v-model="ctx.filter" placeholder="Masukan Nama Tenaga Kerja">
              </b-form-input>
            </div>
          </b-col>
        </b-row>
        <b-table hover responsive id="tenaga-kerja-table" ref="table" :items="itemsProvider"
          ...
          <template #cell(actions)="row">
            <nuxt-link :to="{ name: 'tenaga-kerja.edit', params: { id: row.item.tenaga_kerja_id }}" class="mr-2">
        ...
        <b-row>
          <b-col sm="6">
            <b-pagination v-model="ctx.currentPage" :total-rows="tenaga_kerja_count" :per-page="ctx.perPage"
              aria-controls="tenaga-kerja-table">
            </b-pagination>
          </b-col>
          <b-col sm="6" class="d-flex justify-content-end">
            <span class="font-size">Total Data : {{ tenaga_kerja_count }}</span>
          </b-col>
        </b-row>
      </div>
    </div>
...
export default {
  ...
  head() {
    return { title: 'Tenaga Kerja' }
  },

  async asyncData() {
    if (window.tablectxtenagakerja) {
      var ctx = window.tablectxtenagakerja
    } else {
      var ctx = {
        currentPage: 1,
        perPage: 20,
    ...
    let tenaga_kerja = f1resp.data
    let tenaga_kerja_count = f1resp.count
  ...
  data: () => ({
    fields: [
      { key: 'tenaga_kerja_nama', label: 'Nama Tenaga Kerja', sortable: true, sortDirection: 'asc' },
