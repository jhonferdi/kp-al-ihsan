<template>
  <b-card title="Riwayat Pangkat Baru" class="my-4">
    <br>
    <b-container fluid>
      <b-row class="my-2">
        <b-col sm="3">
...
export default {
  ...
  head() {
    return { title: "Create Riwayat Pangkat" }
  },
  ...
  methods: {
    async save() {
      this.loading = true
      try {
        let resp = (await axios.post('riwayat-pangkat', this.form)).data
        if (resp.success) {
          Swal.fire({
            icon: 'success',
            title: 'Berhasil',
