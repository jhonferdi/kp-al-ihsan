        ...
        <b-tbody>
          <template v-for="(item, i) in datalist">
            <b-tr :key="i">
              ...
              <b-td>{{ item.log_sent_date_formatted }}</b-td>
              <b-td>{{ item.checkin_formatted }}</b-td>
              <b-td>{{ item.checkout_formatted }}</b-td>
              <b-td>{{ item.is_late ? 'Terlambat' : 'Tepat Waktu' }}</b-td>
              <b-td>
                <nuxt-link :to="'/rekap-presensi/view/'+item.peg_id+'/'+item.log_sent_date">
...
export default {
  ...
  async asyncData({ app, params, store }) {
    ...
    searchparams.year = searchparams.year ? searchparams.year : (new Date()).getFullYear();
    searchparams.month = searchparams.month ? searchparams.month : (new Date()).getMonth() + 1;
  ...
  mounted() {
    ...
    if(this.user.satuan_kerja_id && this.user.unit_kerja_id){
      ...
      this.$router.replace({
        path: "/rekap-presensi/" + stringifyParams(searchparams),
      });
  ...
  methods: {
    ...
    async getPegawai(){
      if(this.searchparams.search){
        try{
          let f1 = await axios.get("/rekap-presensi/pegawai-list?search=" + this.searchparams.search);
          let data = f1.data.data;
          console.log(data);
    ...
    async loadData(){
      ...
      let datalist = [];
      let datacount = 0;
      let f1 = await axios.get("/rekap-presensi", {
        params: {
    ...
    pageLink(page) {
      ...
      return { path: "/rekap-presensi/" + stringifyParams(searchparams) };
    },
    sortLink(sort) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    async deleteItem(data, index){
      ...
      await Swal.fire({
        ...
        text: "Menghapus " + data.pegawai.peg_nama + ' - ' + data.log_sent_date_formatted,
        icon: 'warning',
      ...
      await axios.post("/rekap-presensi/delete", {
            id: data.id,
          })
          .then(({ data }) => {
