    ...
    <b-card title="Konfigurasi Kuota Pegawai Shift">
      <div class="form-group row">
        <div class="col-sm-12 text-right">
          <nuxt-link :to="'/konfigurasi-kuota-pegawai-shift/create'">
        ...
        <b-tbody>
          <template v-for="(item, i) in datalist">
            <b-tr :key="i">
              ...
              <b-td>{{ item.shift_name }}</b-td>
              <b-td>{{ item.quota }}</b-td>
              <b-td>{{ item.amount_filled }}</b-td>
              <b-td>{{ item.remainder }}</b-td>
              <b-td>
                <nuxt-link :to="'/konfigurasi-kuota-pegawai-shift/update/'+item.id">
...
export default {
  ...
  methods: {
    async loadData(){
      ...
      let datalist = [];
      let datacount = 0;
      let f1 = await axios.get("/konfigurasi-kuota-pegawai-shift", {
        params: {
    ...
    pageLink(page) {
      ...
      return { path: "/konfigurasi-kuota-pegawai-shift/" + stringifyParams(searchparams) };
    },
    sortLink(sort) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    promptSearch() {
      ...
      this.$router.replace({
        path: "/konfigurasi-kuota-pegawai-shift/" + stringifyParams(searchparams),
      });
    },
    setFilter(col, val) {
      let searchparams = JSON.parse(JSON.stringify(this.searchparams));
    ...
    async deleteItem(data, index){
      ...
      await axios.post("/konfigurasi-kuota-pegawai-shift/delete", {
            id: data.id,
          })
          .then(({ data }) => {
