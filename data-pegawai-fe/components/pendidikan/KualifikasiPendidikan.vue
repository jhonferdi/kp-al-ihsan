<template>
	<div>
	  <div class="d-sm-inline-flex mb-3">
		<div class="d-sm-inline-flex">
		  <b-input-group class="mb-2 mb-sm-0 d-flex" style="max-width:230px">
			<b-form-input ref="search" @keyup.enter="updateSearch()" :value="ctx.filter"
			  placeholder="Cari kualifikasi pendidikan"></b-form-input>
		...
		<!-- <b-button variant="darkgreen" :to="{ name: 'kualifikasi-pendidikan.create' }" size="sm"
		  class="text-nowrap mr-2 px-3 d-inline-flex align-items-center">Tambah Kualifikasi Pendidikan</b-button> -->
		<b-button variant="darkgreen" @click="openModalTambah()" size="sm"
		  class="text-nowrap mr-2 px-3 d-inline-flex align-items-center">Tambah Kualifikasi Pendidikan</b-button>
		<ModalKualifikasiPendidikan :open="modalKualifikasiPendidikanOpen" :pendidikan="pendidikanData" size="xs" :title="`${title} Kualifikasi Pendidikan`"
		  @close="modalKualifikasiPendidikanOpen = false" @onSave="refreshData" />
	  </div>
	  <b-table class="table-rounded" striped hover responsive head-variant="darkgreen" id="kualifikasi-pendidikan-table" ref="table"
		...
		:empty-text="'Tidak ada kualifikasi pendidikan ditemukan'"
		:empty-filtered-text="`Tidak ada kualifikasi pendidikan bernama '${ctx.filter}'`">
	  ...
	  <modal-hapus name="kualifikasi pendidikan" :open="modalHapusOpen" @close="modalHapusOpen = false" @delete="deleteItem" />
	  <TablePagination v-model="ctx.currentPage" :itemsLength="kualifikasi_pendidikan_count" @perPage="getPerPage" />
	</div>
  </template>
<script>
  ...
  import ModalKualifikasiPendidikan from '../modals/ModalKualifikasiPendidikan.vue'
  
  export default {
	middleware: ["auth"],
	data: () => ({
	  ctx: {},
	  kualifikasi_pendidikan: null,
	  kualifikasi_pendidikan_count: null,
	  fields: [
		  { key: "index", label: "No", thClass: 'bg-darkgreen text-white' },
		  { key: 'kualifikasi_pendidikan', tdClass: 'w-100', label: 'Nama Kualifikasi Pendidikan', sortable: true, sortDirection: 'asc', thClass: 'bg-darkgreen text-white' },
		  { key: "actions", label: "Aksi", thClass: 'bg-darkgreen text-white' }
	  ],
	  isTableInit: false,
	  isBusy: false,
	  modalKualifikasiPendidikanOpen: false,
	...
	methods: {
      ...
      if (window.tablectxkualifikasi) {
        this.ctx = window.tablectxkualifikasi
      } else {
        this.ctx = {
          currentPage: 1,
          perPage: 20,
      ...
      this.kualifikasi_pendidikan = f1resp.data
      this.kualifikasi_pendidikan_count = f1resp.count    
	  },
    ...
    async promptDelete(item) {
      ...
      const response = await axios.delete(`/kualifikasi-pendidikan/${this.onDeleteItem.kualifikasi_pendidikan_id}`)
      ...
      this.modalKualifikasiPendidikanOpen = true
	...
	components: { ModalKualifikasiPendidikan }
  }
</script>
