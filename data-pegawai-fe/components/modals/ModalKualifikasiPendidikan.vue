<template>
  <modal :open="open" :size="size" @close="$emit('close')" :title="title">
    ...
    <b-form-input placeholder="Masukkan Kualifikasi Pendidikan" v-model="form.kualifikasi_pendidikan" :state="getErrorState('kualifikasi_pendidikan')">
    </b-form-input>
...
export default {
  ...
  methods: {
    async save() {
      this.loading = true
      try {
        if (this.form.kualifikasi_pendidikan_id) {
          let resp = (await axios.patch('kualifikasi-pendidikan/' + this.form.kualifikasi_pendidikan_id, this.form)).data
          if (resp.success) {
            Swal.fire({
              icon: 'success',
              title: 'Berhasil',
